...
#mono camera from Map
import aslam_cv
def getObservationsFromMap(dataset, mapData):
    print "Extracting map observations"
    # create dictionary with timestamp as index to observations

    obs = mapData['observations']
    mapObs = {}
    for ob in obs:
        obdict = {}
        nameparts = ob['name'].split('.')
        timestamp = float(nameparts[0]+'.'+nameparts[1])
        obdict['position'] = ob['position']
        obdict['orientation'] = ob['orientation']
        obdict['observations'] = ob['observations']
        mapObs[str(timestamp)] = obdict

    mapKeyList = mapObs.keys()
    mapPoints = mapData['structure'] 
    targetObservations = []
    numImages = dataset.numImages()
    
    for timestamp, image in dataset.readDataset():
        # get map entry for the timestamp
        tKey = str(timestamp.toSec())
        if tKey in mapKeyList:
            mapOb = mapObs[tKey]
            observation = aslam_cv.libaslam_cv_python.GridCalibrationTargetObservation()
            t = mapOb['position']
            q = mapOb['orientation']
            rot = matrix_from_quaternion([q['x'], q['y'], q['z'], q['w']])
            tt = np.array([[rot[0,0], rot[0,1], rot[0,2] ,float(t['x'])],
                    [rot[1,0], rot[1,1], rot[1,2] ,float(t['y'])],
                    [rot[2,0], rot[2,1], rot[2,2] ,float(t['z'])],
                    [0,0,0,1.0]])
            tran = sm.Transformation(tt)
            observation.set_T_t_c(tran.inverse())
            observation.setTime(timestamp)
            observation.q = q

            olist = mapOb['observations']
            imagePoints = []
            targetPoints = []
            # take the observations in order of targetID
            klist = [int(k) for k in olist.keys()]
            klist.sort()
            for targetID in klist:
                point2 = olist[str(targetID)]['obs'][0]
                imagePoints.append([float(point2['x']), float(point2['y'])])
                point3 = mapPoints[str(targetID)]
                targetPoints.append([float(point3['x']), float(point3['y']), float(point3['z'])])
            observation.CornersImageFrame =  np.array( imagePoints ).T
            observation.CornersTargetFrame = np.array( targetPoints ).T
...
class IccCameraFromMap():
    def __init__(self, camConfig, mapData, dataset, reprojectionSigma=1.0, showCorners=True, \
        ...
        self.targetObservations = getObservationsFromMap(self.dataset, mapData)
    ...
    def addCameraErrorTerms(self, problem, poseSplineDv, T_cN_b, blakeZissermanDf=0.0, timeOffsetPadding=0.0):
        ...
        for obs in self.targetObservations:
            ...
            # originally returned from a method within the observation:
            #imageCornerPoints =  np.array( obs.getCornersImageFrame() ).T
            #targetCornerPoints = np.array( obs.getCornersTargetFrame() ).T
            imageCornerPoints =  obs.CornersImageFrame
            targetCornerPoints = obs.CornersTargetFrame
